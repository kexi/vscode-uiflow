...
suite('UiFlow Suggest Tests', () => {
  ...
  suiteSetup(() => {
    ...
    fs.copyFileSync(
      path.join(fixtureSourcePath, 'suggest.uif'),
      path.join(fixturePath, 'suggest.uif')
    )
  })
  ...
  test('Test provideCompletionItems#UiflowCompletionItemProvider', async () => {
    ...
    testCases.map(([pos, exp]) => {
      const list = instance.provideCompletionItems(
        doc,
        pos,
        suggest.completionItemCancellationToken
      )
