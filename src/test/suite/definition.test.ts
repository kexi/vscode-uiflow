...
suite('UiFlow Definition Tests', () => {
  ...
  test('Test provideDefinition#UiflowDefinitionProvider', async () => {
    ...
    testCases.forEach(async ([pos, expected]) => {
      const location: vscode.Definition | undefined =
        instance.provideDefinition(
          doc,
          pos,
          definition.definitionCancellationToken
        )
      if (location instanceof vscode.Location) {
        assert.equal(
          location.range.start.line,
          expected,
          `Definition line must be ${expected}.`
        )
      } else {
