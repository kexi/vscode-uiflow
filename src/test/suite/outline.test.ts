...
suite('UiFlow Outline Tests', () => {
  ...
  suiteSetup(() => {
    ...
    fs.copyFileSync(
      path.join(fixtureSourcePath, 'outline.uif'),
      path.join(fixturePath, 'outline.uif')
    )
  })
  ...
  test('Test codeToSections', async () => {
    ...
    assert.equal(
      actual.length,
      expected.length,
      `Section length must be ${expected.length}.`
    )
  ...
  test('provideDocumentSymbols#UiflowDocumentSymbolProvider', async () => {
    ...
    const info = await instance.provideDocumentSymbols(
      doc,
      outline.documentSymbolCancellationToken
    )
    assert.equal(
      info.length,
      expected.length,
      `SymbolInformation length must be ${expected.length}.`
    )
    info.forEach((inf, i) => {
      let ex = expected[i]
      assert.equal(
        inf.name,
        ex.name,
        `SymbolInformation name must be ${ex.name}.`
      )
      assert.equal(
        inf.location.range.start.line,
        ex.lines,
        'SymbolInformation location.range.start.line must be ${ex.lines}.'
      )
    })
  })
})
