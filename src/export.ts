...
export function activate(context: vscode.ExtensionContext) {
  ctx = context
  const d2 = vscode.commands.registerCommand(commandOpenExport, (uri) =>
    openExport(uri)
  )
  const d4 = vscode.commands.registerCommand(commandExportSVG, (uri) =>
    exportAs(uri, CompileFormat.SVG)
  )
  const d5 = vscode.commands.registerCommand(commandExportJSON, (uri) =>
    exportAs(uri, CompileFormat.JSON)
  )
  const d6 = vscode.commands.registerCommand(commandExportDOT, (uri) =>
    exportAs(uri, CompileFormat.DOT)
  )
